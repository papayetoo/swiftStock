...
extension AddStockViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = self.tableView.dequeueReusableCell(withIdentifier: "stock", for: indexPath) as? AddStockViewCell, let data = self.viewModel?.stockInfoData?[indexPath.row] else{return UITableViewCell()}
        cell.name = data.name
        cell.code = data.code
        return cell
    }
